...
def test_csrf_template_fix():
    """Test the CSRF template fix"""
    print("=== Testing CSRF Template Fix ===")
    ...
    try:
        ...
        if 'csrfmiddlewaretoken' in content:
            print("   ❌ CSRF token found - template still has {% csrf_token %}")
            return False
        else:
            print("   ✓ CSRF token removed - template fixed!")
        ...
        print(f"❌ Test CSRF template fix failed: {e}")
        import traceback
        traceback.print_exc()
        return False

def deploy_csrf_template_fix():
    """Deploy the CSRF template fix"""
    print("\n=== Deploying CSRF Template Fix ===")
    ...
    try:
        ...
        # Commit changes
        result = subprocess.run(['git', 'commit', '-m', 'Remove CSRF token from login template'], capture_output=True, text=True)
        if result.returncode != 0:
        ...
        print(f"❌ Deploy CSRF template fix failed: {e}")
        return False
...
def main():
    print("🔧 TESTING AND DEPLOYING CSRF TEMPLATE FIX")
    print("=" * 50)
    
    # Step 1: Test locally
    if not test_csrf_template_fix():
        print("\n❌ Local test failed")
        return False
    
    # Step 2: Deploy
    if not deploy_csrf_template_fix():
        print("\n❌ Deploy failed")
        return False
    ...
    print("\n" + "=" * 50)
    print("🎉 SUCCESS! CSRF template fix deployed and working!")
    print("=" * 50)
